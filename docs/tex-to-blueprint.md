...
```powershell
conda run -n qprint python -m qprint.tex_to_blueprint search --workspace WORKSPACE --query "关键词"
conda run -n qprint python -m qprint.tex_to_blueprint generate --workspace WORKSPACE --tex Paper/main.tex --dest Topology/Paper --title "论文标题" --author "作者" --year 2025 --keyword "拓扑" --dry-run
```
...
初轮全量 Python 回归：`conda run -n qprint pytest -q`，227 项通过；仅有两条既有 FastAPI/Starlette 弃用警告。skill 通过 `quick_validate.py` 校验，并安装到本机默认技能目录；安装后的检索入口已执行验证。

## 独立 skill

所有固定资源均在 skill 内：`references/blueprint-format.md`、`scripts/convert.py` 和 `scripts/qprint_blueprint/` 运行库。skill 运行时不读取产品 prompt、仓库文档或仓库模块；Python 第三方依赖来自 Conda `qprint`。可以在任意目录用绝对路径调用脚本。新增 `validate --workspace WORKSPACE --project PROJECT --render-limit 5` 检查 TeX 绑定、标注覆盖与依赖，并输出渲染 HTML 和警告。
