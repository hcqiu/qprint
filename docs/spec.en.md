...
3. Scope (updated 2026-09-25): all (the entire blueprint folder), current project, or current Milestone (one Markdown file). All granularities allow manual scope selection; level changes preserve scope and the initial default is current project. The reader node supplies initial project/file context, with manual project/file selectors. Milestone scope retains the file's nodes and internal edges at node level, the file at file level, and its owning project at project level. Project level no longer forces all scope. Empty files do not introduce unrelated nodes; refresh preserves valid context and deletion falls back safely.
...
Additional Milestone-entry contract: prioritize the selected Blueprint node in the graph, then the reader node. Synchronize its project/file and immediately reproject, retaining node focus and clearing the old neighborhood filter. A stale manually chosen file must not override the current node. Without a valid node, retain a valid file or fall back safely. Entering Milestone scope from project granularity expands to Blueprint nodes; subsequent manual granularity changes still preserve scope.

Additional acceptance covers unindexed fallback, subfolders, nested indexes, cross-project dependencies, deduplication, empty files, project cycles, both drill-down highlights, all three scopes and edge filtering at each granularity, manual Milestone selection, reader context changes, and refresh/deletion fallback; also following the current node on scope entry, cross-project ownership, stale focus/highlight cleanup, and expansion from project granularity into Milestone scope.
