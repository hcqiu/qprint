...
节点级默认当前项目；文件级允许当前项目或全范围；项目级始终全范围并禁用范围选择。初始项目来自当前阅读节点，也可通过项目选择器手动切换。范围过滤后只保留两端都在范围中的边。
...
`/api/project.graph` 包含 `files`、`projects`、`file_projects`、`node_projects`、`file_edges`、`project_edges`。保存和刷新时重建。前端 `graph-view.js` 用纯函数处理层级、范围、焦点与成员高亮；`graph.js` 负责 SVG、布局和命中区域。
