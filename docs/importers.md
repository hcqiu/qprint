...
`--dest` 可以填父分类 `Topology`，也可以填项目目录 `Topology/Lin20K3`，搭配 `--name Lin20K3` 均生成 `tex/Topology/Lin20K3/` 与 `pdf/Topology/Lin20K3.pdf`。末级目录与论文名一致时不重复追加（不区分大小写）。源码归档若只有包裹整个项目的外壳目录，会去掉该外壳；论文内部的相对路径保持不变，有多个根条目的归档不折叠。已有目标仍拒绝覆盖。
