...
HTML 由 plasTeX 解析 DOM 再通过受控适配器输出段落、标题、定理环境、列表、强调、引用与数学公式；浏览器通过本地 KaTeX 渲染数学。保留论文 preamble 中的宏定义和定理定义。禁止执行 TeX 外部命令/读取外部文件，复杂宏、图像及跨文件 `\\input` 未能在片段渲染时还原的部分给出警告并允许查看原始 TeX；首版不是完整排版引擎，不承诺所有 arXiv 宏包兼容。TeX 原文永远可访问。
...
* 编辑保存整份 Markdown，携带原始 SHA-256，冲突返回 409；写入前校验、原子替换，禁止覆盖外部修改。用户原文件无需转成数据库。
