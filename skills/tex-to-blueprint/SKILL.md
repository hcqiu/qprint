...
在 Qprint 文件夹启动 agent，并保持该工作目录。以下 `SKILL_DIR` 为相对路径 `skills/tex-to-blueprint`；工作区、输入文件、输出文件和引用也都使用相对 Qprint 文件夹的路径，不查找机器上的 Python/Conda 安装路径。宿主准备好本地环境后可用 `conda run -p ./.conda python ...`；若仓库明确指定现有命名环境，则遵循该约定。工具不可用时报告环境尚未准备好。UTF-8 输出可设置当前进程的 `PYTHONIOENCODING=utf-8`、`PYTHONUTF8=1`。顺序启动 Conda，不修改全局配置。
