...
在 Qprint 文件夹启动 agent，并保持该工作目录。以下 `SKILL_DIR` 为相对路径 `skills/tex-to-blueprint`；工作区、输入文件、输出文件和引用也都使用相对 Qprint 文件夹的路径。统一使用 `.\.conda\python.exe`，不 activate、不查找机器上的 Python/Conda 安装路径、不回退到命名或全局环境。若本地解释器缺失，提示用户在 Qprint 根目录的 Anaconda Prompt / Miniconda Prompt 中运行 `conda env create -p .\.conda -f environment.yml`。Conda 可用时等价写法为 `conda run -p .\.conda python ...`。UTF-8 输出可设置当前进程的 `PYTHONIOENCODING=utf-8`、`PYTHONUTF8=1`。
...
```powershell
.\.conda\python.exe "SKILL_DIR/scripts/convert.py" search --workspace WORKSPACE --query "数学对象或标签" --limit 30
```
...
```powershell
.\.conda\python.exe "SKILL_DIR/scripts/convert.py" generate --workspace WORKSPACE --tex Paper/main.tex --dest Topology/Paper --title "论文标题" --author "作者" --year 2025 --keyword "关键词" --dry-run
```
...
```powershell
.\.conda\python.exe "SKILL_DIR/scripts/convert.py" validate --workspace WORKSPACE --project Topology/Paper --render-limit 5
```
