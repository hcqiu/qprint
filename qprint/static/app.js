...
import {followTexReference} from './tex-references.js';
import {referencesHash, referencesRoute, referencesMarkup, citationTarget} from './bibliography.js';
import {createGraphView, syncGraphContext, changeGraphLevel, changeGraphScope, drillGraph, graphProjection} from './graph-view.js';
import {createFocusPublisher, browserSession} from './navigator-state.js';
...
const navigatorSession=browserSession(location.search,()=>window.sessionStorage,()=>crypto.randomUUID());
let navigatorDocument=null;
const publishFocus=createFocusPublisher(payload=>api('/api/navigator/focus',{method:'POST',body:JSON.stringify(payload)}),error=>toast(`Navigator 状态同步失败：${error.message}`));
function syncNavigator(view){if(!state.project?.navigator_available)return;const selection=window.getSelection();const selected=selection?.anchorNode&&$('#content').contains(selection.anchorNode)?String(selection).slice(0,2000):'';return publishFocus({session:navigatorSession,node_id:state.detail?.id??null,view:view??(state.view==='graph'?'graph':state.mode),document:state.detail?(state.mode==='edit'||!state.detail.tex_content?'blueprint/'+state.detail.path:'tex/'+state.detail.tex_content.path):navigatorDocument,selection:selected});}
let toastTimer;
...
async function loadProject(preserve=true){const data=await api('/api/project');state.project=data;$('#project-name').textContent=data.name;$('#node-count').textContent=`${data.stats.nodes} 节点`;$('#progress-label').textContent=`${data.stats.complete} / ${data.stats.nodes}`;$('#progress-bar').style.width=`${data.stats.nodes?data.stats.complete/data.stats.nodes*100:0}%`;$('#diagnostic-count').textContent=data.diagnostics.length;renderTree();
  const references=referencesRoute(location.hash);if(references&&data.papers.some(p=>p.path===references.path)){await openReferences(references.path,references.key,references.from,false);return;}
  ...
  if(chosen){await selectNode(chosen.id,false);history.replaceState(null,'',`#node=${encodeURIComponent(chosen.id)}`);}else{state.selected=null;state.detail=null;navigatorDocument=null;$('#content').innerHTML='<div class="empty-state"><span class="empty-symbol">Q</span><h1>从一个数学对象开始</h1><p>在工作区 blueprint/ 中创建 Markdown 文件，<br>以一级标题定义节点，然后点击刷新。</p></div>';$('#formal-content').innerHTML='<div class="empty-code">没有选中的节点</div>';$('#previous').disabled=true;$('#next').disabled=true;}
...
function renderTree(){if(!state.project)return;const query=$('#search').value.toLowerCase().trim();const nodes=state.project.nodes.filter(n=>`${n.title} ${n.path} ${n.body}`.toLowerCase().includes(query));if(query){$('#tree').innerHTML=`<div class="tree-group"><div class="tree-title">搜索结果 · ${nodes.length}</div>${nodes.map(treeButton).join('')||'<p class="hint">没有匹配的节点</p>'}</div>`;return;}
  const papers=fileTree(state.project.papers,p=>`<details open><summary title="${esc(p.path)}">▤ ${esc(p.name)}</summary>${p.sections.map(s=>`<button class="section-entry" style="padding-left:${14+s.level*8}px" ${s.node_id?`data-node="${esc(s.node_id)}"`:`data-paper="${esc(p.path)}" data-line="${s.line}"`}>${esc(s.title)}</button>`).join('')}${p.has_bibliography?`<button class="section-entry" data-bibliography="${esc(p.path)}">参考文献 / References</button>`:''}${!p.sections.length?`<button class="section-entry" ${p.nodes[0]?`data-node="${esc(p.nodes[0])}"`:`data-paper="${esc(p.path)}"`}>打开论文</button>`:''}</details>`);
...
async function selectNode(id,updateHash=true){if(!canLeave())return false;const request=++state.request;const detail=await api(`/api/node?id=${encodeURIComponent(id)}`);if(request!==state.request)return false;const changed=state.selected!==id;state.selected=id;navigatorDocument=null;syncGraphContext(graphView,state.project,id,changed);state.detail=detail;state.binding=0;state.document=null;state.dirty=false;state.boundary=null;state.lastWheel=performance.now();if(updateHash)setHash(id);renderTree();await renderContent();renderFormal();syncNavigator();return true;}
...
function setView(view){state.view=view;$('#reader').classList.toggle('hidden',view!=='reader');$('#graph').classList.toggle('hidden',view!=='graph');$('#reader-view').classList.toggle('active',view==='reader');$('#graph-view').classList.toggle('active',view==='graph');if(view==='graph')renderGraph();else graph.stop();syncNavigator();}
...
function renderGraph(){
  ...
  $('#graph-project').disabled=!groups.length;
  const files=state.project.graph.files.filter(f=>f.project_id===graphView.projectId);
  const projectPrefix=graphView.projectId==='.'?'':`${graphView.projectId}/`;
  $('#graph-milestone-control').classList.toggle('hidden',graphView.scope!=='milestone');
  $('#graph-milestone').innerHTML=files.map(f=>`<option value="${esc(f.id)}" title="${esc(f.path)}">${esc(f.path.startsWith(projectPrefix)?f.path.slice(projectPrefix.length):f.path)}</option>`).join('');
  $('#graph-milestone').value=graphView.milestoneId??'';
  $('#graph-milestone').disabled=!files.length;
  ...
  const scopeLabel=graphView.scope==='all'?'全范围':graphView.scope==='milestone'?`当前 Milestone：${graphView.milestoneId??'无文件'}`:`当前项目：${graphView.projectId??'无项目'}`;
  $('#graph-context').textContent=`${scopeLabel} · ${graphView.highlightLabel?`${graphView.highlightLabel}：高亮 ${data.highlightIds.length} 个${unit}`:'单击预览，双击'+(graphView.level==='node'?'打开节点':'下钻')}`;
...
async function openPaper(path,line=1){if(!canLeave())return;const data=await api(`/api/tex?path=${encodeURIComponent(path)}`);state.dirty=false;state.detail=null;navigatorDocument='tex/'+path;state.mode='tex';setView('reader');$('#breadcrumb').textContent=`tex / ${path}`;$('#content').innerHTML=`<span class="eyebrow">PAPER SOURCE</span><h2>${esc(path)}</h2><p class="hint">此位置尚无绑定节点。可以在 Markdown 中用 tex 字段关联 bpnode 标签。</p><pre class="tex-source">${data.source.split('\n').map((s,i)=>`<span id="tex-line-${i+1}">${esc(s)}\n</span>`).join('')}</pre>`;$('#formal-content').innerHTML='<div class="empty-code">选择 Blueprint 节点以定位代码</div>';$('#previous').disabled=true;$('#next').disabled=true;$(`#tex-line-${line}`)?.scrollIntoView({block:'start'});syncNavigator('paper');}

async function openReferences(path,key='',from=state.selected,updateHash=true){
  if(!canLeave())return false;
  const request=++state.request;
  const data=await api(`/api/references?path=${encodeURIComponent(path)}`);
  if(request!==state.request)return false;
  from=state.project.nodes.some(n=>n.id===from)?from:'';
  state.dirty=false;state.detail=null;navigatorDocument='tex/'+path;state.document=null;state.mode='rendered';state.boundary=null;
  if(updateHash){const hash=referencesHash(path,key,from);if(location.hash!==hash)history.pushState(null,'',hash);}
  setView('reader');
  $('#breadcrumb').textContent=`tex / ${path} / References`;
  $('#position').textContent='REFERENCES';
  $('#previous').disabled=true;$('#next').disabled=true;
  document.querySelectorAll('[data-mode]').forEach(b=>b.classList.remove('active'));
  $('#formal-content').innerHTML='<div class="empty-code">论文参考文献</div>';
  const content=$('#content');content.innerHTML=referencesMarkup(data,from);math(content);content.scrollTop=0;
  const anchor=citationTarget(data,key);if(anchor)document.getElementById(anchor)?.scrollIntoView({block:'center'});
  syncNavigator('references');
  return true;
}

let selectionTimer;
document.addEventListener('selectionchange',()=>{clearTimeout(selectionTimer);selectionTimer=setTimeout(()=>{if(state.detail)syncNavigator();},200);});

document.addEventListener('click',guarded(async e=>{const citation=e.target.closest('[data-bibliography]');if(citation){e.preventDefault();await openReferences(citation.dataset.bibliography,citation.dataset.citeKey||'');return;}const texRef=e.target.closest('a.tex-ref');if(texRef){e.preventDefault();await followTexReference(texRef,{currentNode:state.selected,selectNode,openPaper,showReader:()=>setView('reader'),findAnchor:id=>document.getElementById(id)});return;}const node=e.target.closest('[data-node]');if(node){if(await selectNode(node.dataset.node))setView('reader');return;}const paper=e.target.closest('[data-paper]');if(paper){await openPaper(paper.dataset.paper,Number(paper.dataset.line)||1);return;}const binding=e.target.closest('[data-binding]');if(binding){state.binding=Number(binding.dataset.binding);renderFormal();return;}const mode=e.target.closest('[data-mode]');if(mode&&state.detail){if(!canLeave())return;state.dirty=false;state.mode=mode.dataset.mode;await renderContent();return;}const open=e.target.closest('[data-graph-open]');if(open){await openGraphItem(open.dataset.graphOpen);return;}const link=e.target.closest('a.wikilink');if(link){e.preventDefault();if(await selectNode(decodeURIComponent(link.hash.slice(6))))setView('reader');}}));
...
$('#graph-scope').onchange=()=>{changeGraphScope(graphView,state.project,$('#graph-scope').value,state.selected);$('#graph-local').checked=false;renderGraph();};
...
$('#graph-milestone').onchange=()=>{graphView.milestoneId=$('#graph-milestone').value;graphView.highlightIds=[];graphView.highlightLabel=null;graphView.focusId=null;renderGraph();};
...
window.addEventListener('popstate',guarded(async()=>{const references=referencesRoute(location.hash);if(references){if(!await openReferences(references.path,references.key,references.from,false))setHash(state.selected);return;}const id=decodeURIComponent(location.hash.replace(/^#node=/,''));if(state.project?.nodes.some(n=>n.id===id)){if(await selectNode(id,false))setView('reader');else setHash(state.selected);}}));
...
window.addEventListener('focus',()=>syncNavigator());
document.addEventListener('visibilitychange',()=>{if(!document.hidden)syncNavigator();});
