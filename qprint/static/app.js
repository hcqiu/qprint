...
document.querySelectorAll('.close-dialog').forEach(b=>b.onclick=()=>b.closest('dialog').close());$('#import-open').onclick=()=>$('#import-dialog').showModal();$('#import-kind').onchange=()=>{const paper=$('#import-kind').value==='paper';$('#code-options').classList.toggle('hidden',paper);$('#paper-name').classList.toggle('hidden',!paper);$('#paper-name input').required=paper;$('#source-label').firstChild.textContent=paper?'arXiv 编号或地址':'仓库 URL';$('#source-label input').placeholder=paper?'2001.00001 或 https://arxiv.org/abs/…':'https://github.com/owner/repository';};
$('#import-form').addEventListener('submit',guarded(async e=>{e.preventDefault();const values=Object.fromEntries(new FormData(e.target));values.ref=values.ref||null;const button=$('#import-submit');button.disabled=true;$('#import-result').textContent='正在提交下载任务…';try{const job=await api('/api/import',{method:'POST',body:JSON.stringify(values)});let finished=false;while(!finished){await new Promise(resolve=>setTimeout(resolve,1200));const current=await api(`/api/jobs/${job.id}`);$('#import-result').textContent={queued:'等待下载…',running:'正在下载和校验文件…',failed:`导入失败：${current.error}`,succeeded:`导入完成\n${JSON.stringify(current.result,null,2)}`}[current.status];finished=['succeeded','failed'].includes(current.status);if(current.status==='succeeded'){if(!state.dirty)await loadProject();else toast('资料已导入；保存当前编辑后刷新索引');}}}finally{button.disabled=false;}}));
